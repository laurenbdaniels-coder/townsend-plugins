...
- Git via subprocess only with `git -c core.fsmonitor= -c core.hooksPath=/dev/null -c core.pager=cat -C <repo>`: `ls-files -z -- '.env*' '*/.env*'` first (index-only), then `for-each-ref --count=1000 refs/tags`, `rev-parse --is-shallow-repository`, and last `rev-list --count --exclude-promisor-objects HEAD`, under one 15 s budget, with `GIT_*` variables stripped, `GIT_NO_LAZY_FETCH=1`, hooks, pager, fsmonitor, ssh and credential helpers blanked, and a git directory refused outright when it carries pointers, a per-worktree config, symlinks or special files anywhere under refs/logs/objects, or, after a strict parse that refuses a stray carriage return, a continuation line or a key sharing its section header's line, any directive that could send git outside the tree (`core.worktree`, `include`/`includeIf`, `extensions.partialClone`/`worktreeConfig`, `remote.*.promisor`/`uploadpack`, command-running keys); ordinary furniture (submodules, LFS, GUI settings, credential helpers) passes, and a refusal reports `git-config-not-vouched` with `git-index-unread` on Q1 and marks the scan partial; the enclosing repository is found and checked in Python before git runs, and `GIT_CEILING_DIRECTORIES` pins discovery to it; a regular `.git/shallow` → `shallow:true`.
...
2. **Failure envelope.** On failure the scanner prints `{ok:false, error:<code>, hint:<text>, docs:<README anchor>, partial:true}` with stable codes `usage`, `repo-not-found`, `repo-not-a-directory`, `repo-is-symlink`, `repo-unreadable`, `repo-contains-cwd`, `python-too-old`, `internal:<ExceptionClass>`. Every hint is path-free and names the next action.
3. **`version` and `stats`.** The success JSON gains top-level `version` (the scanner's `__version__`, the single source for `--version`, the stderr line, the verdict footer and the CI version guard) and `stats` (`files_skipped_oversize`, `files_skipped_binary`, `files_skipped_generated`, `files_never_open`, `files_skipped_special`, `files_skipped_hardlink`, `files_errored`, `dirs_unreadable`, `dirs_truncated`, `mcp_capped`, `git_index_partial`, `output_trimmed`, `max_files_hit`, `max_total_bytes_hit`, `deadline_hit`, `config`).
