...
It runs inside Claude Code or Codex. It never changes your app. It never sends anything anywhere.
...
- The scanner never opens your AI instruction files: `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.cursorrules`, `.windsurfrules`, `.clinerules`, `.claude/`, `.codex/`, `.agents/`, `.cursor/rules/`, `.windsurf/`, `.github/instructions/`, `copilot-instructions.md`. The footer tells you how many it skipped.
- It never follows a symlink, never reads a lockfile, a source map, or a minified bundle, never reads a file over 512 KB, and stops at 20,000 files or 256 MB or two minutes and says so (`partial: yes`).
...
Workshop-supported: macOS and Linux. Windows: scanner unit tests pass in CI; the skill run is untested.
