...
- **Build folders.** A `build`, `dist` or `out` folder is still not read, but it now keeps Q1, Q3 and Q9 off Nothing found, with one `build-output-unread` row listing the folders; `.next`, `.nuxt` and a `vendor` folder below the root count too. Photos, fonts, archives and other binaries are no longer opened at all. Found by running the check on a real prototype whose whole source lived in `build/`: it had reported "nothing found" after reading one file.
